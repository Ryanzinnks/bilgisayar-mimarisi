...
\bolumfoto{gorseller/bolum03-suleymaniye.jpg}{Süleymaniye Camisi -- İstanbul}
...
Gerçekte iki taraf birbirinden tamamen ayrışmadı. Modern x86 işlemciler (Intel Pentium Pro'dan itibaren, 1995) karmaşık CISC buyrukları içeride basit mikro-işlemlere (\emph{micro-ops}) dönüştürür; boru hattı bu mikro-işlemler üzerinde çalışır. Dışarıdan CISC görünen işlemci içeride RISC gibi davranmaktadır.
...
Herhangi bir işlem ya da veri boyutu ne kadar düzenliyse ve kural dışı durumlardan ne kadar arındırılmışsa, donanımdaki uygulaması o kadar yalındır. Daha yalın olan donanımın karmaşıklığı düşüktür ve tasarımı kolaydır.
\end{tanim}
...
Bölüm~\ref{chap:basarim}'de tanıttığımız bu ilke burada somut biçimde karşımıza çıkar. RISC-V tasarımcıları, programcıya çok sayıda yazmaç sunarken yazmaç birimini tek çevrimde erişilebilecek kadar küçük tutmayı hedeflemiştir. 32~yazmaç bu dengenin sonucudur: 16~yazmaç derleyiciler için yetersiz kalır; 64~yazmaç ise yazmaç dosyasını büyüterek erişim süresini uzatır ve buyruk içindeki adres alanını genişleterek buyruk boyutunu artırır.
\end{tanim}
...
Tablo~\ref{tab:yazmac-karsilastirma}'de görüldüğü gibi mimari tasarımlar arasında yazmaç sayısı önemli ölçüde farklılık gösterir. Bu fark rastlantısal değildir; yazmaç sayısı hem başarımı hem de buyruk kodlama biçimini doğrudan etkiler. Her iki uçtaki sorunları anlamak, mimari tasarım kararlarının ardındaki mantığı kavramak açısından önemlidir.
...
\item 32 yazmaçlı mimari: 5 buyruk (yalnızca 1 bellek erişimi), saat sıklığı 3{,}8~GHz (daha büyük yazmaç dosyası nedeniyle biraz düşük).
...
\centering
\begin{tikzpicture}[scale=1.0]
  % Buyruk kutusu (sola hizalı, x=3 merkez)
  \node[font=\small, above=0.05cm] at (3, 0.5) {Buyruk};
  \node[draw, thick, fill=blokmavi-acik, minimum width=5cm, minimum height=1cm,
        font=\small, align=center] (buyruk) at (3, 0) {};
  % rs alanı vurgusu
  \draw[thick, fill=blokyesil] (3.8, -0.5) rectangle (5.3, 0.5);
  \node[font=\small] at (4.55, 0) {rs1};
  % Yazmaç Öbeği
  \node[draw, thick, fill=bloksari, minimum width=2.5cm, minimum height=2.5cm,
        font=\small\bfseries, align=center] (yazkut) at (9, -2.5) {Yazmaç\Öbeği};
  % İşlenen (sonuç)
  \node[draw, thick, fill=blokyesil, minimum width=2cm, minimum height=0.8cm,
        font=\small, rounded corners=3pt] (islened) at (12.0, -2.5) {İşlenen};
  % Oklar
  \draw[okstil, thick, sinyalmavi] (4.55, -0.5) -- (4.55, -2.5) -- (yazkut.west);
  \node[font=\footnotesize, sinyalmavi, right] at (4.7, -1.5) {yazmaç numarası};
...
\begin{tikzpicture}[scale=1.0]
  % Buyruk kutusu
  \node[draw, thick, fill=blokmavi-acik, minimum width=6cm, minimum height=1cm,
        font=\small, align=center] (buyruk) at (0, 0) {};
  \node[font=\small, above=0.15cm] at (0, 0.5) {Buyruk};
  % imm (anlık) alanı vurgusu
  \draw[thick, fill=blokkirmizi] (-2.8, -0.5) rectangle (-0.3, 0.5);
  \node[font=\small] at (-1.55, 0) {imm};
  % rs1 alanı
  \draw[thick, fill=blokyesil] (-0.3, -0.5) rectangle (1.0, 0.5);
  \node[font=\small] at (0.35, 0) {rs1};

  % İşlenen (doğrudan)
  \node[draw, thick, fill=blokyesil, minimum width=2cm, minimum height=0.8cm,
        font=\small, rounded corners=3pt] (islenen) at (6, -1.5) {İşlenen};

  % Ok: anlık değer doğrudan işlenene gider
  \draw[okstil, thick, sinyalkirmizi] (-1.55, -0.5) -- (-1.55, -1.5) -- (islenen.west)
        node[midway, below, font=\footnotesize] {anlık değer doğrudan kullanılır};

  % Etiket
  \node[font=\small, text=gray, align=center] at (2, -2.5) {Anlık değer buyruğun içinde gömülüdür};
\end{tikzpicture}
...
esizebox{\linewidth}{!}{%
\begin{tikzpicture}[scale=0.75]
  % Buyruk kutusu (yazı üstte)
  \node[draw, thick, fill=blokmavi-acik, minimum width=6cm, minimum height=1cm,
        font=\small, align=center] (buyruk) at (0, 0) {};
  \node[font=\small, above=0.15cm] at (0, 0.5) {Buyruk};
  % imm alanı vurgusu
  \draw[thick, fill=blokkirmizi] (-2.8, -0.5) rectangle (-0.8, 0.5);
  \node[font=\small] at (-1.8, 0) {imm};
  % rs1 alanı
  \draw[thick, fill=blokyesil] (-0.8, -0.5) rectangle (0.7, 0.5);
  \node[font=\small] at (-0.05, 0) {rs1};

  % Yazmaç Öbeği (sola alındı)
  \node[draw, thick, fill=bloksari, minimum width=1.8cm, minimum height=1.5cm,
        font=\small\bfseries, align=center] (yazkut) at (3.5, -1.8) {Yazmaç\\Öbeği};

  % Toplama birimi
  \node[draw, thick, circle, fill=blokgri, minimum size=0.8cm, font=\small] (top) at (7.5, -1.8) {$+$};

  % Bellek
  \node[draw, thick, fill=blokyesil, minimum width=2cm, minimum height=2cm,
        font=\small\bfseries, align=center] (bellek) at (10.5, -1.8) {Bellek};

  % İşlenen
  \node[draw, thick, fill=blokyesil, minimum width=1.5cm, minimum height=0.7cm,
        font=\small, rounded corners=3pt] (islenen) at (13.5, -1.8) {Veri};

  % Oklar: rs1 alanının altından aşağı, dik dönüşle yazmaç öbeğine
  \draw[okstil, thick, sinyalmavi] (-0.05, -0.5) |- (yazkut.west)
        node[near end, above, font=\footnotesize] {yazmaç numarası};
  \draw[okstil, thick, sinyalmavi] (yazkut.east) -- (top.west)
        node[midway, above, font=\footnotesize] {taban adresi};
  % imm alanının altından aşağı çıkıp, toplayıcının altından bağlanır
  \draw[okstil, thick, sinyalkirmizi] (-1.8, -0.5) -- (-1.8, -3.5) -| (top.south);
  \node[font=\footnotesize, sinyalkirmizi, above] at (2.85, -3.5) {eklenti (bayt)};
  ...
  % Formül
  \node[font=\small, text=vurgumavi] at (5, -4.5) {Adres = rs1 + eklenti $\rightarrow$ Bellek[Adres]};
\end{tikzpicture}
}%resizebox
...
\centering
\begin{tikzpicture}[scale=1.0]
  % Buyruk kutusu (yazı üstte, kutu yukarı alındı)
  \node[draw, thick, fill=blokmavi-acik, minimum width=5cm, minimum height=1cm,
        font=\small, align=center] (buyruk) at (0, 0.8) {};
  \node[font=\small, above=0.15cm] at (0, 1.3) {Buyruk};
  % imm alanı vurgusu
  \draw[thick, fill=blokkirmizi] (-2.3, 0.3) rectangle (-0.3, 1.3);
  \node[font=\small] at (-1.3, 0.8) {imm};

  % PS yazmacı
  \node[draw, thick, fill=bloksari, minimum width=2cm, minimum height=0.8cm,
        font=\small\bfseries] (ps) at (0, -1.8) {PS};

  % Toplama birimi
  \node[draw, thick, circle, fill=blokgri, minimum size=0.8cm, font=\small] (top) at (5, -0.5) {$+$};

  % Hedef Adres (sağa alındı)
  \node[draw, thick, fill=blokyesil, minimum width=2.5cm, minimum height=0.8cm,
        font=\small\bfseries, rounded corners=3pt] (hedef) at (10, -0.5) {Hedef Adres};

  % Oklar
  \draw[okstil, thick, sinyalkirmizi] (-1.3, 0.3) |- (top.west)
  ...
  % Formül
  \node[font=\small, text=vurgumavi] at (4.5, -2.8) {Hedef Adres = PS + eklenti};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=1.0]
  % lui buyruğu
  \node[font=\small, above=0.05cm] at (0, 0.5) {\texttt{lui} Buyruğu};
  \node[draw, thick, fill=blokmavi-acik, minimum width=6cm, minimum height=1cm] (buyruk) at (0, 0) {};
  % imm[31:12] alanı (sol kenar buyruk kutusuna yapışık: -3.0)
  \draw[thick, fill=blokkirmizi] (-3.0, -0.5) rectangle (0.8, 0.5);
  \node[font=\small] at (-1.1, 0) {imm[31:12]};
  % rd alanı
  \draw[thick, fill=blokyesil] (0.8, -0.5) rectangle (2.0, 0.5);
  \node[font=\small] at (1.4, 0) {rd};
  % opcode alanı
  \draw[thick, fill=blokgri!30] (2.0, -0.5) rectangle (3.0, 0.5);
  \node[font=\scriptsize] at (2.5, 0) {opcode};

  % Ok: imm alanının ortasından aşağı (üst ve alt kutuların merkezi -1.1)
  \draw[okstil, thick, sinyalkirmizi] (-1.1, -0.5) -- (-1.1, -2.2);

  % Hedef yazmaç etiketi (solda)
  \node[font=\small\bfseries, anchor=east] at (-3.2, -2.5) {Hedef Yazmaç (rd):};

  % Bit numaraları (kutunun üstünde)
  \node[font=\scriptsize] at (-3.0, -2.05) {31};
  \node[font=\scriptsize] at (0.8, -2.05) {12};
  \node[font=\scriptsize] at (1.1, -2.05) {11};
  \node[font=\scriptsize] at (3.0, -2.05) {0};

  % 32-bit yazmaç gösterimi (imm kısmı üstteki ile hizalı: -3.0 → 0.8)
  \draw[thick, fill=blokkirmizi!30] (-3.0, -2.8) rectangle (0.8, -2.2);
  \node[font=\small] at (-1.1, -2.5) {imm[31:12]};
  \draw[thick, fill=blokgri!30] (0.8, -2.8) rectangle (3.0, -2.2);
  \node[font=\small] at (1.9, -2.5) {0000...0};

  % İki aşamalı yapı gösterimi
  \node[font=\small, text=vurgumavi] at (0, -3.3) {\textbf{1.} \texttt{lui rd, üst\_20} \quad $\Rightarrow$ \quad rd = \texttt{üst\_20} $\ll$ 12};
  \node[font=\small, text=vurgumavi] at (0, -3.9) {\textbf{2.} \texttt{addi rd, rd, alt\_12} \quad $\Rightarrow$ \quad rd = tam 32 bit sabit};
\end{tikzpicture}
...
\end{tikzpicture}
\caption{A programı B altyordamını, B C'yi, C de D'yi çağırır -- yığıt durumu}
...
\end{tikzpicture}
\caption{Program çalıştığı anda yığıtın durumu}
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[font=\small\bfseries, anchor=west] at (\cw+1.1, 1.5*\ch) {PS};
\end{tikzpicture}
