...
2000'li yılların ortalarından itibaren güç duvarı\index{güç duvarı} ve Dennard ölçeklemesinin\index{Dennard ölçeklemesi} sona ermesi saat sıklığını artırma stratejisini sürdürülemez kıldı. Tasarımcılar, tek bir yonga üzerine birden çok çekirdek\index{çok çekirdekli} yerleştirerek başarım artışını paralel hesaplamaya kaydırdı. Ancak paralel hesaplamanın da sınırları vardır: Gene Amdahl\index{Amdahl, Gene}'ın 1967'de ortaya koyduğu \emph{Amdahl Yasası}\index{Amdahl Yasası}, bir programın sıralı kalan kısmının toplam hızlanmayı sınırladığını gösterir. Bu yasa Bölüm~\ref{chap:basarim}'de ayrıntılı olarak incelenecektir. Günümüzde CPU çekirdeklerinin yanına GPU\index{GPU}, NPU\index{NPU} ve diğer özelleşmiş hızlandırıcılar eklenerek \emph{heterojen} mimariler oluşturuluyor. Açık kaynaklı RISC-V\index{RISC-V} buyruk kümesi mimarisi de bu dönemin önemli gelişmeleri arasında yer alıyor.
...
\begin{tikzpicture}[>=Stealth, x=0.22cm, y=1.15cm]
    ...
    % Arrow Lake: 2024, 3e10 -> log10=10.477 -> y=7.477
    ...
    \filldraw[sinyalmavi] (2022, 7.415) circle (2.5pt)
        node[below right, font=\scriptsize] {Raptor Lake};
    \filldraw[sinyalmavi] (2024, 7.477) circle (2.5pt)
        node[above right, font=\scriptsize] {Arrow Lake};
...
\begin{ornek}[1.2]
Çapı 300\,mm olan bir silisyum plakanın maliyeti 12\,000\,\$ olsun. Üretilecek yonganın alanı 200\,mm$^2$, birim alan başına hata sayısı 0{,}03\,hata/mm$^2$ ve kritik aşama sayısı $N=4$ ise bir yonganın maliyetini hesaplayın.
...
Verim: $\displaystyle\left(\frac{1}{1 + 0{,}03 \times 200}\right)^{4} = \left(\frac{1}{7}\right)^{4} \approx 4{,}2 \times 10^{-4}$ --- bu son derece düşük bir verimdir ve yüksek hata yoğunluğunun etkisini göstermektedir.

Gerçek üretimde birim alan başına hata sayısı çok daha düşüktür. Eğer hata yoğunluğu 0{,}001\,hata/mm$^2$ olsaydı:
\[
\text{Verim} = \left(\frac{1}{1 + 0{,}001 \times 200}\right)^{4} = \left(\frac{1}{1{,}2}\right)^{4} \approx 0{,}482
\]
Bu durumda yonga maliyeti: $\displaystyle\frac{12\,000}{353 \times 0{,}482} \approx 70{,}5$\,\$.

Bu örnek, hata yoğunluğunun maliyet üzerindeki dramatik etkisini açıkça gösteriyor: aynı yonga alanında hata yoğunluğunun 30 kat azalması, verimi sıfıra yakın bir değerden \%48'e çıkarıyor.
\end{ornek}
...
Önceleri bütün işlemler doğrudan makine dili\index{makine dili} kullanılarak yapılıyordu. Daha sonra insan mantığına ve konuşma biçimine uygun yollar geliştirildi. Komutlar yalnızca sayılarla değil, kelimeler kullanılarak da bilgisayara iletildi. Bilgisayarlara emir vermek için ikilik tabandaki sayılar yerine küçük anlamlı sözcükler kullanan bu dillere \emph{çevirici dil}\index{çevirici dil} (\emph{Assembly}) denir. Makine kodlarına en yakın kod çevirici dil kodudur. Bu kod, yazıldıktan sonra makine üzerinde kurulmuş çeviricilerle (\emph{Assembler}) tekrar makine diline dönüştürülür. Örneğin çevirici dilde \texttt{add x5, x6, x7} yazıldığında, çevirici program bu kodu \texttt{0000000\allowbreak 00111\allowbreak 00110\allowbreak 000\allowbreak 00101\allowbreak 0110011} olarak 32 bitlik makine diline çevirir.\footnote{Bu, RISC-V'in R-tipi buyruk biçimidir; Bölüm~\ref{chap:buyruk-kumesi}'te ayrıntılı olarak açıklanacaktır.}
...
Bu kitapta buyruk kümesi mimarisi olarak açık kaynaklı \textbf{RISC-V}\index{RISC-V} kullanılıyor. RISC-V'in neden tercih edildiği, tescilli mimarilerle karşılaştırması ve endüstrideki yeri Bölüm~\ref{sec:neden-riscv}'de ayrıntılı olarak ele alınıyor.
\end{bilginot}
...
\begin{tikzpicture}[>=Stealth, node distance=1.8cm]
    ...
    \node[blok, fill=bloksari, minimum width=7cm, minimum height=1.4cm, below=0.6cm of ceviriciprg] (makine)
        {\textbf{Makine Dili}\\[2pt]\texttt{0000000\,00111\,00110\,000\,00101\,0110011}};
...
Şekil~\ref{fig:isa-karsilastirma}'da üç önemli buyruk kümesi mimarisinin temel boyutlarda karşılaştırması veriliyor. Günümüzde SiFive, Alibaba T-Head, Andes ve NVIDIA gibi firmalar RISC-V tabanlı ticari işlemciler üretiyor. Qualcomm\index{Qualcomm}, giyilebilir aygıtlarında RISC-V çekirdeklerine geçiş yapmaya başladı. Google, Android\index{Android} işletim sistemine RISC-V desteği ekledi ve bu hamle RISC-V'in mobil ekosistemde yaygınlaşmasının önünü açıyor. Avrupa İşlemci Girişimi (European Processor Initiative) de yüksek başarımlı hesaplama alanında RISC-V'e yatırım yapıyor. Akademik dünyada ise BOOM, Rocket ve CVA6 gibi açık kaynaklı RISC-V çekirdekleri araştırmacılara gerçek bir işlemci tasarımı üzerinde deney yapma olanağı sunuyor.
...
\begin{tcolorbox}[colback=blokmavi-acik, colframe=sinyalmavi!70!black, title={\textbf{Anahtar Kavramlar}}, fonttitle=\bfseries]
\small
\begin{multicols}{3}
\begin{itemize}[nosep, leftmargin=*]
\item Bilgisayar mimarisi
\item Buyruk kümesi mimarisi (ISA)
\item Von Neumann mimarisi
\item Harvard mimarisi
\item RISC / CISC
\item RISC-V
\item Moore Yasası
\item Dennard ölçeklemesi
\item Transistör / Tümleşik devre
\item Yonga / Kırmık
\item Soyutlama
\item Fotolitografi
\item Teknoloji düğümü
\item Verim
\item Makine dili / Çevirici dil
\item Derleyici / İşletim sistemi
\item Devingen güç tüketimi
\item Durağan güç tüketimi
\item TDP
\item Güç duvarı
\item Çok çekirdekli mimari
\item Heterojen hesaplama
\item GPU / TPU / NPU
\item Alana özgü mimari
\item Amdahl Yasası
\end{itemize}
\end{multicols}
\end{tcolorbox}
